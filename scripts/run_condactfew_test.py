...
from scripts.Dataset_test import MolData
from model.model_ace_noleakage import MolNet
from scripts.utils import get_logger, set_seed, loss_rela, load_data_singlevalid, metrics
...
def training(model, train_loader, optimizer, loss_f, metric, device, task_i, prompt_i, typ):
    ...
    for data in train_loader:
        if data.y.size()[0] > 1:
            ...
            weights = torch.where(typs, torch.ones_like(typs), torch.ones_like(typs)*0.1).to(device)

            if relation is None:
                loss = loss_f(logits.squeeze(), y.squeeze())*weights
            else:
                loss = loss_f(logits.squeeze(), y.squeeze(), relation)*weights
            loss = loss.mean()
...
def main(modelparm, dataset, device, train_epoch, seed, fold, batch_size, rate, modelpath, logger, lr, met, savem):
    ...
    trainset = MolData(root='./CondACT_few', dataset=f'{dataset}_train_pg')
    testset = MolData(root='./CondACT_few', dataset=f'{dataset}_test_pg')

    test_smis = set([d.smi for d in testset])
    orig_train_len = len(trainset)
    filtered_train = [d for d in trainset if d.smi not in test_smis]
    removed = orig_train_len - len(filtered_train)
    if removed > 0:
        logger.info(f"Removed {removed} samples from trainset because they appear in testset")
    trainset = filtered_train
    ...
    for fol in range(1, fold+1):
        ...
        params_group = [ 
            {'params': [p for n, p in model.named_parameters() if ('emb' in n) or ('emb_org' in n)], 'lr': lr*0.1}, 
            {'params': [p for n, p in model.named_parameters() if ('emb' not in n) and 'emb_org' not in n], 'lr': lr} 
        ] 
        ...
        train_loader, valid_loader, test_loader = load_data_singlevalid(trainset, testset, batch_size, rate[0], seed_, typ)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_epoch', type=int, default=50, help='Number of epochs to train (default: 30)')
    ...
    parser.add_argument('--task_file', type=str, default='CondACT_few/raw/ace_few_task.npy', help = "Path of the npy file containing task embedding") 
    ...
    logf = 'log/ACT_Few_{}.log'.format(moldata)
