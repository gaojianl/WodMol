...
from utils import get_logger, set_seed, loss_rela, load_data_singlevalid, metrics
...
def main(modelparm, dataset, device, train_epoch, seed, fold, batch_size, rate, modelpath, logger, lr, met, savem):
    ...
    dataset_path = os.path.join(project_root, 'CondACT_few')
    ...
    for fol in range(1, fold+1):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=train_epoch, eta_min=1e-4, last_epoch=-1)
