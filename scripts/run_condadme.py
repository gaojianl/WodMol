...
from utils import get_logger, set_seed, loss_rela, load_data, metrics


acelist = ['F', 'T12', 'Cmax', 'Stability', 'CL']


def training(model, train_loader, optimizer, loss_f, metric, device, task_i, prompt_i):
...
def main(modelparm, dataset, device, train_epoch, seed, fold, batch_size, rate, modelpath, logger, lr, met, savem):
    ...
    for fol in range(1, fold+1):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=train_epoch, eta_min=lr_min, last_epoch=-1)
        ...
        for i in range(train_epoch):
            loss, train_mae = training(model, train_loader, optimizer, loss_f, metric, device, task_i, None)
            logger.info(f"Epoch {i + 1}/{train_epoch}  Train Loss: {loss:.4f} MAE: {train_mae:.4f}")
            
            loss, valid_mae = testing(model, valid_loader, loss_f, metric, device, False, task_i, None)
            ...
            if valid_mae < best_val_rmse:
                loss, test_mae = testing(model, test_loader, loss_f, metric, device, False, task_i, None)
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--moldata', type=str, default='Cl', help='Dataset name')
    ...
    parser.add_argument('--pretrain', type=str, default='checkpoints/model.pkl', help = "Path of retrained weights")
    parser.add_argument('--task_file', type=str, default='CondADME/raw/adme_task.npy', help = "Path of the npy file containing task embedding") 
    ...
    logf = 'log/ADME_{}.log'.format(moldata, moldata) 
