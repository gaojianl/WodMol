...
def main(modelparm, dataset, pi, device, train_epoch, seed, fold, batch_size, rate, modelpath, logger, lr, met, savem):
    ...
    for fol in range(1, fold+1):
        ...
        for i in range(train_epoch):
            loss, train_mae = training(model, train_loader, optimizer, loss_f, metric, device, None, pi, typ)
            logger.info(f"Epoch {i + 1}/{train_epoch}  Train Loss: {loss:.4f} MAE: {train_mae:.4f}")
            
            if sche:
                scheduler.step()
            
            loss, valid_mae = testing(model, valid_loader, loss_f, metric, device, False, None, pi, typ)
            ...
            if valid_mae < best_val_rmse:
                loss, test_mae = testing(model, test_loader, loss_f, metric, device, False, None, pi, typ)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pi', type=str, default=None, help='Task id. For unseen task, use None')
    parser.add_argument('--seed', type=int, default=426, help = "Seed for splitting the dataset")
    parser.add_argument('--pretrain', type=str, default='checkpoints/model_CSLoss.pkl', help = "Path of retrained weights")
    parser.add_argument('--task_file', type=str, help = "Path of the npy file containing task embedding") 
